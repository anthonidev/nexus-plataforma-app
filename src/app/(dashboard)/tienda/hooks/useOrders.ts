...
import {
  getMyOrdersClient,
  OrderFilters,
} from "@/lib/actions/ecommerce/ecommerce-client.action";
import { ItemOrder } from "@/types/ecommerce/client/ecommerce.types";

export function useOrders(initialPage: number = 1, initialLimit: number = 10) {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      const response = await getMyOrdersClient(filters);
